...
##
## Does this:
## 1. reads the Elwha_MR_*bin0.25_regrid_cc files into an xarray
## 2. reads the Elwha_MR_im_time_mean_prob image into an xarray
...
for counter,g in tqdm(enumerate(geometries)):
    ...
    plt.axis('off')
    plt.savefig(f"../results/MR/MR_wood/summary/wood_animation/Wood_01_frame_{counter}_alltime.png", dpi=300, bbox_inches='tight')
    plt.close()
    del wood_da
    ...
    plt.savefig(f"../results/MR/MR_wood/summary/wood_animation/Wood_01_frame_{counter}_occupancy_alltime.png", dpi=300, bbox_inches='tight')
    plt.close()
    del sum_da

    tmp = wood_c.wood.sum("time", skipna=True)
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/Elwha_MR_region_{counter}_wood_sum_time.tif", dtype=dtype)
    del tmp
...
if run_bash:
    ...
    for i in range(len(geometries)):
        try:
            os.mkdir(f"../results/MR/MR_wood/summary/wood_animation/region{i}")
        except:
            pass
        os.system(f'mv ../results/MR/MR_wood/summary/wood_animation/Wood_01_frame_{i}*.png ../results/MR/MR_wood/wood_animation/region{i}')

        os.system(f'convert -delay 100 ../results/MR/MR_wood/summary/wood_animation/region{i}/Wood_01_frame_{i}*.png ../results/MR/MR_wood/wood_animation/wood_animation_region{i}.gif')
...
for counter,g in tqdm(enumerate(geometries)):
    ...
    for inner_counter, time in enumerate(times[1:]):
        ...
        tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/Elwha_MR_region_{counter}_wood_diff_time0.tif", dtype=dtype)
        del tmp
...
# plt.axis('off')
# plt.savefig(f"../results/MR/MR_wood/summary/wood_animation/Wood_MR_frame_{counter}_allreach_alltime.png", dpi=300, bbox_inches='tight')
# plt.close()
# del wood_da
# del tmp_da
...
#     for i in range(len(geometries)):
#         try:
#             os.mkdir(f"../results/MR/MR_wood/summary/region{i}")
#         except:
#             pass
#         os.system(f'mv ../results/MR/MR_wood/summary/Wood_01_frame_{i}*.png ../results/MR/MR_wood/summary/wood_animation/region{i}')

#         os.system(f'convert -delay 100 ../results/MR/MR_wood/summary/wood_animation/region{i}/Wood_01_frame_{i}*.png wood_animation_region{i}.gif')
