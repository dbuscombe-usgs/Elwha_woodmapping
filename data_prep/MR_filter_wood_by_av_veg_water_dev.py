...
sed_files = sorted(glob('../raw_data/MR/MR_all/MR_*sed*_regrid.tif'))
...
wood2_files = sorted(glob('../raw_data/MR/MR_all/MR_*wood2*_regrid.tif'))
...
############################################################
# Load in and concatenate all individual GeoTIFFs for sediment
...
for counter,g in tqdm(enumerate(geometries)):
    ...
    try:
        ...
        os.mkdir(f"../results/MR/MR_sed/region{counter}")
    except:
        pass
    ...
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_sed/region{counter}/Elwha_MR_region_{counter}_sed_time_mean_bin.tif", dtype=dtype)
    del tmp

    tmp = sed_c.var("time", skipna=True)
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_sed/region{counter}/Elwha_MR_region_{counter}_sed_time_var_bin.tif", dtype=dtype)
    del tmp
...
# wood2_geotiffs_ds = wood2_geotiffs_ds.drop_vars(2)
...
water_mask_ds = rioxarray.open_rasterio("../results/MR/MR_water/Elwha_MR_water_time_bin0.5_regrid.tif", chunks=chunksize, dtype='uint8')
water_mask_ds = water_mask_ds.to_dataset('band')

# sed_mask_ds = rioxarray.open_rasterio("../results/MR/MR_sed/Elwha_MR_sed_time_bin0.9_regrid.tif", chunks=chunksize, dtype='uint8')
# sed_mask_ds = sed_mask_ds.to_dataset('band')

# dist_files = sorted(glob("../results/MR/MR_dist2braid/*.tif"))
...
### filter wood
# print(wood_geotiffs_ds.sum().compute()) ##1.819e+07

wood_geotiffs_ds = wood_geotiffs_ds.where((veg_mask_ds[1] < 1))
# print(wood_geotiffs_ds.sum().compute()) ## 1.758e+07
wood_geotiffs_ds = wood_geotiffs_ds.where((water_mask_ds[1] < 1))
# print(wood_geotiffs_ds.sum().compute()) ##
...
for counter,g in tqdm(enumerate(geometries)):
    ...
    try:
        os.mkdir(f"../results/MR/MR_wood/summary/region{counter}")
    except:
        pass
    ...
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/region_{counter}_wood_time_var_prob.tif", dtype=dtype)
    del tmp

    tmp = wood_c.mean("time", skipna=True)
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/region_{counter}_wood_time_mean_prob.tif", dtype=dtype)
    del tmp

    for time in times:
        tmp = wood_c.sel(time=time)
        tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/MR_{time}_region_{counter}_wood_prob.tif", dtype=dtype)
        # del tmp        
...
for counter,g in tqdm(enumerate(geometries)):
    ...
    tmp = wood_c.var("time", skipna=True)
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/region_{counter}_wood2_time_var_prob.tif", dtype=dtype)
    del tmp

    tmp = wood_c.mean("time", skipna=True)
    tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/region_{counter}_wood2_time_mean_prob.tif", dtype=dtype)
    del tmp

    for time in times:
        tmp = wood_c.sel(time=time)
        tmp.rio.to_raster(raster_path=f"../results/MR/MR_wood/summary/region{counter}/MR_{time}_region_{counter}_wood2_prob.tif", dtype=dtype)
        del tmp        
...
if run_bash:
    ...
    os.chdir(f"../results/MR/MR_wood/summary")
    os.system("bash mosaic_timeaverage.sh")
    ...
    os.system("bash clip_all_wood.sh")
    os.system("bash clip_all2_wood.sh")
